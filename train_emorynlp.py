...
def train_or_eval_model(model, model2, model3, loss_function, ESCL, CKSCL, dataloader, epoch, optimizer=None, train=False):
    ...
    for data in dataloader:
        ...
        log_prob, _, alpha, alpha_f, alpha_b, _ = model(r1, r2, r3, r4, x5, x6, x1, o2, o3, qmask, umask, False, False)
...
def train_or_eval_model2(model, model2, model3, loss_function, ESCL, CKSCL, dataloader, epoch, optimizer=None, train=False):
    ...
    for data in dataloader:
        ...
        loss3 = CKSCL(lp_scl, lac_scl, weight=create_class_weight_SCL(lac_scl)) #context
        loss4 = CKSCL(lp_scl, lac2_scl, weight=create_class_weight_SCL(lac2_scl)) # kb
        #############################################################################################################

        if train:
            total_loss = loss + 0.3 * loss2 + 0.3 * loss3 + 0.6 * loss4
            total_loss.backward()
            if args.tensorboard:
                for param in model.named_parameters():
                    writer.add_histogram(param[0], param[1].grad, epoch)
    ...
    # if train:
    #     print(np.sum(masks), sum(cccs), sum(kkks))
...
if __name__ == '__main__':
    ...
    #model = torch.load('\model_emorynlp_emo3.pkl')
    ...
    loss_weights2 = torch.FloatTensor([0.5, 1, 1, 0.3, 1, 1, 0.9])
    ...
    for e in range(n_epochs):
        ...
        if test_fscore[-1] > best_test_f1:
            ...
            #torch.save(model, '\empirical_model_emorynlp3_use_None.pkl')
